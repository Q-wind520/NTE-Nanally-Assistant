...
def script_DiaoYu(times):
    ...
    while True:
        if wait_image_appear(str(base / 'DiaoYu.png')) != None: break
    pdi.press('f')
    click(str(base / 'start.png'))
    if wait_image_appear(str(base / 'lacking.png'), timeout=1):
            print("Script: 鱼饵已耗尽")
            pdi.press('esc')
            return -1
    time.sleep(2)
    for i in range(times):
        ...
        # 上鱼
        try:
            wait_image_appear(str(base / 'upfish.png'), timeout=20, confidence=0.7)
        except:
            print("Script: 鱼儿逃走了")
            i -= 1
            continue
        pdi.press('f')
        time.sleep(2)
        wait_image_appear(str(base / 'close.png'), timeout=20, confidence=0.7)
        time.sleep(0.5)
        pdi.click()
    pdi.press('esc')
    time.sleep(1)
